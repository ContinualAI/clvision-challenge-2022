...
class ImprovedLRSchedulerPlugin(SupervisedPlugin):
    ...
    def __init__(
        ...
        step_granularity='epoch', first_epoch_only=False, first_exp_only=False
    ):
        """
        ...
        :param step_granularity: defines how often the scheduler's `step()`
            ...
            'epoch' and 'iteration'.
        :param first_epoch_only: if True, the scheduler will only be considered
        ...
        warnings.warn(
            'The granularity control features currently provided by '
            ' ImprovedLRSchedulerPlugin will'
            ' be merged in Avalanche\'s LRSchedulerPlugin (master branch) by '
        ...
        if self.step_granularity == 'iteration' and self.metric == 'val_loss':
            raise ValueError(
                'Incompatible step_granularity and metric: the "iteration" '
                'granularity can only be used with metric == None or '
                '"train_loss"')

        ImprovedLRSchedulerPlugin._patch_lr_on_plateau(self.scheduler)
    ...
    def after_training_iteration(
        ...
        if self.step_granularity == 'iteration':
            if self.metric == "train_loss":
                self.scheduler.step(metrics=self.rolling_metric.result())
            else:
                self.scheduler.step()
